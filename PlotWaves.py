...
for file in os.listdir( userInput.get('dataSource') ):
    ...
    Y = np.array(Y).astype(float).reshape(len(X)) / 1000  # convert to km

    breaks = np.where(np.diff(Y) >= 1)[0]
    ...
    for i in range(len(X)):
        if userInput.get('plot3D'):
            X[i] = mdates.date2num(X)
            ax.plot(X[i], np.zeros(len(X[i])), Y[i], color='blue')
        else:
